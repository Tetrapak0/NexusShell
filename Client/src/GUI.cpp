...
int gui_init() {
	...
	io.Fonts->AddFontFromMemoryCompressedBase85TTF(Roboto_compressed_data_base85, 16, NULL, glyph_range);
...
void draw_home() {
    ...
    if (!reconfiguring.load() && !profiles.empty() && !CURRENT_PROFILE.buttons.empty()) {
        for (int i = 0; i < CURRENT_PROFILE.size; ++i) {
            ImGui::BeginDisabled(CURRENT_BUTTOM_LOOP.action.empty());
            if (CURRENT_BUTTOM_LOOP.default_label) draw_button(to_string(i+1), i);
            else if (CURRENT_BUTTOM_LOOP.label.empty()) draw_button("##", i);
            else draw_button(CURRENT_BUTTOM_LOOP.label, i);
...
void draw_killed() {
    ...
    if (ImGui::BeginPopupModal("Killed", NULL, im_window_flags)) {
        ...
        ImGui::Text("%s", kill_reason.c_str());
