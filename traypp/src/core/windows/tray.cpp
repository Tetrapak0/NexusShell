...
#include <Windows.h>
#include "../../../include/core/entry.hpp"
#include "../../../include/core/windows/tray.hpp"
#include <stdexcept>

#include "../../../include/components/button.hpp"
#include "../../../include/components/imagebutton.hpp"
#include "../../../include/components/label.hpp"
#include "../../../include/components/separator.hpp"
#include "../../../include/components/submenu.hpp"
#include "../../../include/components/syncedtoggle.hpp"
#include "../../../include/components/toggle.hpp"
...
Tray::Tray::Tray(std::wstring identifier, Icon icon) : BaseTray(std::move(identifier), icon) {
    ...
    if (RegisterClassEx(&windowClass) == 0) throw std::runtime_error("Failed to register class");

    // NOLINTNEXTLINE
    hwnd = CreateWindow(this->identifier.c_str(), nullptr, 0, 0, 0, 0, 0, nullptr, nullptr, windowClass.hInstance, nullptr);
    if (hwnd == nullptr) throw std::runtime_error("Failed to create window");

    if (UpdateWindow(hwnd) == 0) throw std::runtime_error("Failed to update window");
    ...
    if (Shell_NotifyIcon(NIM_ADD, &notifyData) == FALSE) throw std::runtime_error("Failed to register tray icon");
...
void Tray::Tray::exit() {
    ...
    DestroyMenu(menu);
    UnregisterClass(this->identifier.c_str(), GetModuleHandle(nullptr));
...
void Tray::Tray::update() {
    ...
    if (Shell_NotifyIcon(NIM_MODIFY, &notifyData) == FALSE)   throw std::runtime_error("Failed to update tray icon");
...
HMENU Tray::Tray::construct(const std::vector<std::shared_ptr<TrayEntry>> &entries, Tray *parent, bool cleanup) {
    static auto id = 0;
    if (cleanup)  parent->allocations.clear();
    ...
    for (const auto &entry : entries) {
        ...
        winItem.wID = ++id;
        std::string string_identifier(name.get());
        std::wstring wstring_identifier = std::wstring(string_identifier.begin(), string_identifier.end());
        winItem.dwTypeData = wstring_identifier.data();
        ...
        if (auto *toggle = dynamic_cast<Toggle *>(item); toggle) {
            if (toggle->isToggled())    winItem.fState |= MFS_CHECKED;
            else                        winItem.fState |= MFS_UNCHECKED;
        } else if (auto *syncedToggle = dynamic_cast<SyncedToggle *>(item); syncedToggle) {
            if (syncedToggle->isToggled())  winItem.fState |= MFS_CHECKED;
            else                            winItem.fState |= MFS_UNCHECKED;
        ...
        } else if (dynamic_cast<Label *>(item))     winItem.fState = MFS_DISABLED;
        else if (dynamic_cast<Separator *>(item))   winItem.fType = MFT_SEPARATOR;

        if (!dynamic_cast<Label *>(item))  if (item->isDisabled())  winItem.fState = MFS_DISABLED;
