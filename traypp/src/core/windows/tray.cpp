...
Tray::Tray::Tray(std::string identifier, Icon icon) : BaseTray(std::move(identifier), icon)
{
    ...
    wchar_t* wchar_identifier = (wchar_t*)malloc(sizeof(wchar_t) * this->identifier.length()+1);
    std::mbstowcs(wchar_identifier, this->identifier.c_str(), this->identifier.length());
    windowClass.lpszClassName = wchar_identifier;
    ...
    // NOLINTNEXTLINE
    hwnd = CreateWindow(wchar_identifier, nullptr, 0, 0, 0, 0, 0, nullptr, nullptr, windowClass.hInstance,
                        nullptr);
    free(wchar_identifier);
    if (hwnd == nullptr)
    {
...
void Tray::Tray::exit()
{
    ...
    DestroyMenu(menu);
    wchar_t* wchar_identifier = (wchar_t*)malloc(sizeof(wchar_t) * identifier.length() + 1);
    std::mbstowcs(wchar_identifier, identifier.c_str(), identifier.length());
    UnregisterClass(wchar_identifier, GetModuleHandle(nullptr));
...
HMENU Tray::Tray::construct(const std::vector<std::shared_ptr<TrayEntry>> &entries, Tray *parent, bool cleanup)
{
    ...
    for (const auto &entry : entries)
    {
        ...
        wchar_t* wchar_identifier = (wchar_t*)wstring_identifier.c_str();
        winItem.dwTypeData = wchar_identifier;
